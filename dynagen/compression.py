...
class TorchCompressedDevice:
    ...
    def allocate(self, shape, dtype, comp_config, pin_memory=None, name=None):
        ...
        data_shape = shape[:group_dim] + (num_groups * (group_size // 2),) + shape[group_dim + 1 :]
        scale_shape = shape[:group_dim] + (num_groups, 2) + shape[group_dim + 1 :]
        ...
        return TorchTensor(shape, np_dtype_to_torch_dtype[dtype], (data, scale, comp_config), self, name=name)
    ...
    def init_cache_one_gpu_batch(self, config, task, policy):
        ...
        pin_memory = False
        k_cache = self.allocate(shape, np.float16, comp_config=policy.comp_cache_config, pin_memory=pin_memory)
        v_cache = self.allocate(shape, np.float16, comp_config=policy.comp_cache_config, pin_memory=pin_memory)
    ...
    def init_attention_compute_workspace(self, config, task, policy):
        ...
        group_size, group_dim = (policy.comp_cache_config.group_size, policy.comp_cache_config.group_dim)
        num_groups = (shape[group_dim] + group_size - 1) // group_size
    ...
    def compress(self, tensor, comp_config):
        ...
        group_size, num_bits, group_dim, symmetric = (
            comp_config.group_size,
            comp_config.num_bits,
            comp_config.group_dim,
            comp_config.symmetric,
        )
        ...
        if pad_len != 0:
            ...
            tensor = torch.cat(
                [tensor, torch.zeros(pad_shape, dtype=tensor.dtype, device=tensor.device)], dim=group_dim
            )
        ...
        # Pack
        left_indices = tuple(slice(0, x) for x in data.shape[: group_dim + 1]) + (
            slice(0, data.shape[group_dim + 1], 2),
        )
        right_indices = tuple(slice(0, x) for x in data.shape[: group_dim + 1]) + (
            slice(1, data.shape[group_dim + 1], 2),
        )
        data = torch.bitwise_or(data[left_indices].bitwise_left_shift(4), data[right_indices])
        ...
        return TorchTensor(shape, tensor.dtype, (data, scale, comp_config), self)
    ...
    def decompress(self, tensor):
        ...
        # Pad
        new_shape = shape[:group_dim] + (num_groups, group_size_c) + shape[group_dim + 1 :]
        pad_len = (group_size_c - shape[group_dim] % group_size_c) % group_size_c
        if pad_len != 0:
            pad_shape = shape[:group_dim] + (pad_len,) + shape[group_dim + 1 :]
            data = torch.cat([data, torch.zeros(pad_shape, dtype=data.dtype, device=data.device)], dim=group_dim)
        ...
        if unpad_len != 0:
            flatten_shape = shape[:group_dim] + (num_groups * group_size,) + shape[group_dim + 1 :]
...
def general_copy_compressed(dst, dst_indices, src, src_indices):
    assert src.device.device_type == DeviceType.COMPRESSED and dst.device.device_type == DeviceType.COMPRESSED

    src_data_indices, src_scale_indices = get_compressed_indices(src, src_indices, src.shape)

    dst_data_indices, dst_scale_indices = get_compressed_indices(dst, dst_indices, dst.shape)
...
def get_compressed_indices(tensor, indices, shape):
    ...
    data_indices[group_dim] = slice(indices[group_dim].start // 2, (indices[group_dim].stop + 1) // 2)

    scale_indices = indices
    scale_indices.insert(group_dim + 1, slice(0, 2))
    scale_indices[group_dim] = slice(
        indices[group_dim].start // group_size, (indices[group_dim].stop + group_size - 1) // group_size
    )

    return data_indices, scale_indices


default_cache_config = CompressionConfig(num_bits=0, group_size=0, group_dim=0, symmetric=False, enabled=False)
